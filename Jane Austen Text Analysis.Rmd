...
ggplot(marry_data, aes(ch)) +
  geom_histogram(aes(fill = factor(para)), binwidth = 1)+
...
```{r}
tnum.tagByQuery("*pride* has text = REGEXP(\"family\")", "reference:family")
family <- tnum.query("*pride* has text = REGEXP(\"family\")", max = 155) 
family_df <- tnum.objectsToDf(family)
#View(family_df)
picfamily <- tnum.makePhraseGraphFromPathList(love_df$subject)
tnum.plotGraph(picfamily)
```
```{r}
tnum.tagByQuery("*pride* has text = REGEXP(\"darcy\")", "reference:darcy")
darcy <- tnum.query("*pride* has text = REGEXP(\"darcy\")", max = 150) 

darcy_df <- tnum.objectsToDf(darcy)
#View(family_df)
# picdarcy <- tnum.makePhraseGraphFromPathList(darcy_df$subject)
# tnum.plotGraph(picdarcy)
```
